...
// >> Argument of type '{hex: string;}' is not assigneable to parameter of type 'Point'.
// >> Type '{hex: string;}' is missing the following properties from type 'Point': x, y.

// We can see from the above example that TypeScript offers more precise error catching than it's ancestor, JavaScript.

    // There is no difference between how classes and objects conform to shapes:

class VirtualPoint { // (Instead of an interface...)
    x: number;
    y: number;
